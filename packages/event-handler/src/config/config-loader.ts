 ...
 */
export async function validateConfigAdvanced(
    config: Config,
    options: {
    ...
    await validateConfiguration(config, options);
}
...
function validateConfig(config: Config): void {
    ...
    if (!config.nats.consumerDurable) {
        throw new Error('Configuration missing "nats.consumerDurable"');
    }
    ...
    // Set defaults
    config.eventMappings = config.eventMappings || [];
